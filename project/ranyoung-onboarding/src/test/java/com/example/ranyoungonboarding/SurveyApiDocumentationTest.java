...
public class SurveyApiDocumentationTest {
        ...
        mockMvc.perform(post("/api/surveys")
                ...
                .andDo(document("create-survey",
                        ...
                        requestFields(
                                ...
                                fieldWithPath("questions[].options").optional().description("선택 항목 (단일 또는 다중 선택 리스트인 경우)")
                        ),
                        ...
                        responseFields(
                                ...
                                fieldWithPath("description").description("설문조사 설명"),
                                fieldWithPath("questions").description("설문 항목 목록"),
                                fieldWithPath("questions[].id").optional().description("항목 ID"),
                                fieldWithPath("questions[].name").description("항목 이름"),
                                fieldWithPath("questions[].description").description("항목 설명"),
                                fieldWithPath("questions[].type").description("항목 입력 형태"),
                                fieldWithPath("questions[].required").description("필수 항목 여부"),
                                fieldWithPath("questions[].options").optional().description("선택 항목 목록"),
    ...
    public void submitResponseTest() throws Exception {
        ...
        answer1.setQuestionId(questionId1); // 예시 질문 ID
        answer1.setValue("매우 만족");
        answers.add(answer1);

        AnswerRequest answer2 = new AnswerRequest();
        answer2.setQuestionId(questionId2); // 예시 질문 ID
    ...
    public void getSurveyResponsesTest() throws Exception {
        ...
        mockMvc.perform(get("/api/surveys/{surveyId}/responses", surveyId)
                ...
                .andDo(document("get-survey-responses",
                        ...
                        responseFields(
                                fieldWithPath("content").type(JsonFieldType.ARRAY).optional().description("검색된 응답 목록"),
                                fieldWithPath("content[].id").type(JsonFieldType.NUMBER).optional().description("응답 ID"),
                                fieldWithPath("content[].surveyId").type(JsonFieldType.NUMBER).optional().description("설문조사 ID"),
                                fieldWithPath("content[].answers").type(JsonFieldType.ARRAY).optional().description("응답 항목 목록"),
                                fieldWithPath("content[].answers[].id").type(JsonFieldType.NUMBER).optional().description("응답 항목 ID"),
                                fieldWithPath("content[].answers[].questionId").type(JsonFieldType.NUMBER).optional().description("질문 ID"),
                                fieldWithPath("content[].answers[].questionName").type(JsonFieldType.STRING).optional().description("질문 이름"),
                                fieldWithPath("content[].answers[].textValue").type(JsonFieldType.STRING).optional().description("응답 값"),
                                fieldWithPath("content[].answers[].multipleValues").type(JsonFieldType.ARRAY).optional().description("다중 선택 응답 값"),
                                fieldWithPath("content[].submittedAt").type(JsonFieldType.STRING).optional().description("제출 시간"),
        ...
        mockMvc.perform(get("/api/surveys/{surveyId}/responses", surveyId)
                ...
                .andDo(document("get-survey-responses",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("content[].id").type(JsonFieldType.ARRAY).optional().description("응답 ID"),
                                fieldWithPath("content[].surveyId").type(JsonFieldType.NUMBER).optional().description("설문조사 ID"),
                                fieldWithPath("content[].answers").type(JsonFieldType.ARRAY).optional().description("응답 항목 목록"),
                                fieldWithPath("content[].answers[].id").type(JsonFieldType.NUMBER).optional().description("응답 항목 ID"),
                                fieldWithPath("content[].answers[].questionId").type(JsonFieldType.NUMBER).optional().description("질문 ID"),
                                fieldWithPath("content[].answers[].questionName").type(JsonFieldType.STRING).optional().description("질문 이름"),
                                fieldWithPath("content[].answers[].textValue").type(JsonFieldType.STRING).optional().description("응답 텍스트"),
                                fieldWithPath("content[].answers[].multipleValues").type(JsonFieldType.ARRAY).optional().description("다중 응답 값"),
                                fieldWithPath("content[].submittedAt").type(JsonFieldType.STRING).optional().description("응답 제출 시간"),
    ...
    public void setUp(WebApplicationContext webApplicationContext,
        ...
        SurveyItemRequest q2 = new SurveyItemRequest();
        q2.setName("개선사항");
        q2.setDescription("서비스 개선을 위한 제안사항");
        q2.setType(QuestionType.LONG_ANSWER);
        q2.setRequired(false);

        request.setQuestions(List.of(q1, q2));
        ...
        questionId1 = root.get("questions").get(0).get("id").asLong();
        questionId2 = root.get("questions").get(1).get("id").asLong();
    }
}